...
def render_daily_brief_tab():
    ...
    if cached_brief is None:
        with st.spinner("🔍 Running Sherlock Analysis... (this takes 30-60 seconds)"):
...
def _generate_brief(df_weekly: pd.DataFrame, seed_asin: str, target_brand: str) -> Optional[DailyBrief]:
    ...
    try:
        ...
        st.write("📊 Transforming data to event stream...")
        ...
        st.write(f"✅ Generated {summary.get('total', 0)} events ({summary.get('portfolio_events', 0)} yours, {summary.get('competitor_events', 0)} competitors)")
        ...
        if brief and brief.narratives:
            ...
            st.write("📝 Saved predictions to journal for tracking")
...
def _render_daily_brief(brief: DailyBrief):
    ...
    if brief.journal_context:
        with st.expander("📓 Journal: Your Prediction History"):
