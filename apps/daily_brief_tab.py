...
def render_daily_brief_tab():
    ...
    st.caption("Strategic narratives from your AI Analyst • ShelfGuard Intelligence (Gen 3/4)")
    ...
    if df_weekly.empty and project_asins and SUPABASE_LOADER_AVAILABLE:
        with st.spinner("📦 Loading historical data from cache..."):
            df_weekly = load_weekly_timeseries(tuple(project_asins), days=90)
            if not df_weekly.empty:
                st.session_state['df_weekly'] = df_weekly
                st.caption(f"✅ Loaded {len(df_weekly)} weekly records from cache")
    ...
    with col3:
        if st.button("⚡ Quick Vitals", use_container_width=True):
    ...
    if cached_output is None:
        with st.spinner("🔍 Running Causal Reasoning Engine..."):
...
def _render_no_project_state():
    ...
    with st.expander("ℹ️ What the Causal Reasoning Engine Does"):
        st.markdown("""
...
def _show_quick_vitals(df_weekly: pd.DataFrame, asin: str):
    ...
    with col1:
        ...
        color = {"HEALTHY": "🟢", "AT_RISK": "🟡", "CRITICAL": "🔴"}.get(health, "⚪")
    ...
    if vitals.get("trends"):
        ...
        for metric, direction in vitals["trends"].items():
            icon = "📈" if direction == "GROWING" else "📉" if direction == "DECLINING" else "➡️"
...
def _generate_analysis(
    ...
    try:
        ...
        # Get market context
        st.write("📅 Loading world context...")
        ...
        st.write("🧠 Running Causal Reasoning Engine...")
        st.write("  → Profiler (Triage)")
        st.write("  → Calibrator (Learning Physics)")
        st.write("  → Volatility Sensor (Anomalies)")
        st.write("  → Prediction Engine (Forecasting)")
...
def _render_orchestrator_output(output: OrchestratorOutput):
    ...
    if output.brief:
        ...
        with col1:
            ...
            icon = health_icons.get(summary.overall_health, "⚪")
    ...
    if output.brief:
        with st.expander("🔬 Diagnostic Brief Details"):
...
def _render_narrative(narrative: StrategicNarrative, index: int):
    ...
    color, icon = type_colors.get(narrative.narrative_type, ("#3b82f6", "📈"))
    ...
    with tab1:
        if brief.profiler_vitals:
            ...
            for name, vital in brief.profiler_vitals.target_vitals.items():
                status_icon = "✅" if vital.is_good else "⚠️" if vital.severity in ["MINOR", "MODERATE"] else "🚨"
    ...
    with tab3:
        if brief.forecast_signal:
            ...
            for metric, forecast in brief.forecast_signal.forecasts.items():
                ...
                if change:
                    st.write(f"  • {metric}: {change:+.1%} expected")
